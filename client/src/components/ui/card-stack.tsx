...
export function CardStack<T extends CardStackItem>({
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between px-4 sm:px-8 pointer-events-none z-50 w-full max-w-[600px] mx-auto">
                <button
                    onClick={(e) => { e.stopPropagation(); prev(); }}
                    className="pointer-events-auto bg-white/90 dark:bg-black/90 hover:bg-white p-3 rounded-full shadow-xl hover:scale-110 transition-all duration-200 backdrop-blur-sm border border-black/5"
...
function DefaultFanCard({ item }: { item: CardStackItem; active: boolean }) {
    return (
        <div className="relative h-full w-full">
            ...
            {/* Content with Frost Effect */}
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 flex flex-col justify-end z-20">
                <div className="backdrop-blur-md bg-white/10 border border-white/10 rounded-2xl p-4 shadow-lg">
                    <div className="text-xl md:text-2xl font-semibold italic text-[#d4e600] mb-2 drop-shadow-md leading-tight font-serif tracking-wide">
                        {item.title}
                    </div>
                    {item.description ? (
                        <div className="text-base md:text-lg text-white font-medium leading-relaxed drop-shadow-sm font-sans">
                            {item.description}
                        </div>
                    ) : null}
                </div>
